	...
	def update(date)
	    ...
	    doc.css("tbody")[0].css("tr").each do |item|
	      ...
	      if player
			...
			puts "player exists #{player.full_name}"
			playerName = item.at_css(".col-name span").text.rstrip
			playerNameArray = playerName.split(", ")

		    player.update_attributes(:full_name => item.at_css(".col-name span").text.rstrip)
	        player.update_attributes(:first_name => playerNameArray[1].rstrip)
	        player.update_attributes(:last_name => playerNameArray[0].rstrip)
	        player.update_attributes(:jersey => item.at_css(".col-jersey").text)
	        player.update_attributes(:roster_positon => item.at_css(".col-position").text)
	        ...
	        player.update_attributes(:experience => playerExp.rstrip)
	        player.update_attributes(:college => item.at_css(".col-college").text)
	        player.update_attributes(:player_full_image => "#{playerNameArray[0]}_#{playerNameArray[1]}")
	        player.update_attributes(:player_image => "#{playerNameArray[0]}_#{playerNameArray[1]}_thumb.png")
            ...
            p.short_id = arrKeys.last
			p.roster_id = pID
			playerName = item.at_css(".col-name span").text.rstrip
			playerNameArray = playerName.split(", ")
			playerExp = item.at_css(".col-exp").text.lstrip 
			p.full_name = item.at_css(".col-name span").text.rstrip
			p.first_name = playerNameArray[1].rstrip
			p.last_name = playerNameArray[0].rstrip
	        p.last_update = updateTime
			p.jersey = item.at_css(".col-jersey").text
			p.roster_position = item.at_css(".col-position").text
			p.height = item.at_css(".col-height").text
			p.weight = item.at_css(".col-weight").text
			p.age = item.at_css(".col-bd").text
			p.experience = playerExp.rstrip
			p.college = item.at_css(".col-college").text
			p.player_full_image = "#{playerNameArray[0]}_#{playerNameArray[1]}.png"
			p.player_image = "#{playerNameArray[0]}_#{playerNameArray[1]}_thumb.png"
...
# 	      	createStats(p)
	...
	def removePlayers(updatePlayerdate)
		puts Roster.all.count
		Roster.delete_all(["last_update < ?", updatePlayerdate])
