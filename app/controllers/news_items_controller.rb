class NewsItemsController < ApplicationController
  ...
  # GET /news_items.json
  def index
    @news_items = NewsItem.where("ignore = ?", false).order("pub_date DESC").page(params[:page]).per(10)
    # @news_items.find_all_by_ignore(false)
    # @news_items = NewsItem.order("pub_date DESC").find_all_by_ignore(false)
